...
function add_applicant (obj, num) {
    const applicant = `<div class="applicant">
                        ...
                        <div class="ngay"><p class="content">${obj.ngayphongvan}</p></div>
...
async function get_interviewer (p) {
    await $.get("/noidung/interview", (data) => {
        var count = 0, page = 0;
        data.forEach(element => {
            const ngayungtuyen_date = new Date(element['ngayphongvan']);
            const ngayungtuyen = ngayungtuyen_date.toLocaleDateString('en-GB');
            applicants[count] = element;
            applicants[count]['ngayphongvan'] = ngayungtuyen;
...
async function open_update_cv_status (id) {
    ...
    await $.get(`/noidung/applicants/${id}`, (data) => {
        ...
        const ngayungtuyen = ngayungtuyen_date.toLocaleDateString('en-GB');
        const ngayphongvan_date = new Date(data['ngaynhanviec']);
        const ngayphongvan = ngayphongvan_date.toLocaleDateString('en-GB');
        $("#cv_status").val(data['trangthai']);
        (ngayungtuyen_date.getTime() < ngayphongvan_date.getTime()) ? $("#cv_interviewday").val(data['ngaynhanviec']) : $("#cv_interviewday").val('');
...
function check_update_diffrence () {
    const payload = {
        ...
        'ngaynhanviec': $("#cv_interviewday").val()? $("#cv_interviewday").val() : applicants_update.ngaynhanviec,
        'ghichu': $("#cv_notice").val()
    }
    const trangthai = payload.trangthai == applicants_update.trangthai;
    const ngayphongvan = (new Date(payload.ngaynhanviec)).getTime() == (new Date(applicants_update.ngaynhanviec)).getTime()
...
async function submit_update_applicant (id) {
    ...
    var ngayphongvan_date = !$("#cv_interviewday").val() ? new Date(applicants_update['ngaynhanviec']) : new Date($("#cv_interviewday").val());
    const payload = {
        'trangthai': $("#cv_status").val(),
        'ngaynhanviec': ngayphongvan_date.toLocaleDateString('zh-Hans-CN'),
        'ghichu': $("#cv_notice").val()
    };
    const check = check_update_diffrence();
    if (check) {
        $.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}})
        await $.post (`/noidung/interview/${id}`, {payload: payload})
            .done((data) => {
                if (data == 'success') {
                    alert("Thông báo đã được cập nhật.")
                    close_update_cv_status();
                    $("#applicant_pagination").replaceWith('<div id="applicant_pagination" class="pagination"></div>');
                    get_interviewer(cur_page);
                }
                else {
