...
function add_applicant (obj, num) {
    const applicant = `<div class="applicant">
                        ...
                        <div class="ngay"><p class="content">${obj.ngaynhanviec}</p></div>
...
async function get_interviewer (p) {
    await $.get("/noidung/intern", (data) => {
        var count = 0, page = 0;
        data.forEach(element => {
            const ngayungtuyen_date = new Date(element['ngaynhanviec']);
            const ngayungtuyen = ngayungtuyen_date.toLocaleDateString('en-GB');
            applicants[count] = element;
            applicants[count]['ngaynhanviec'] = ngayungtuyen;
